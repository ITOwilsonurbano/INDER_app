...
public class ApiDocumentTypes {
    ...
    DocumentTypesListener callback;
    Context context;

    public ApiDocumentTypes(Fragment fragment){
        try {
            callback = (DocumentTypesListener) fragment;
        } catch (ClassCastException e) {
            throw new ClassCastException("Calling fragment must implement DocumentTypesListener interface");
        }
        this.context = fragment.getActivity().getApplicationContext();
    }

    public void getDocumentTypes (HashMap params){
        ...
        JsonObjectRequest jsonObjReq = new JsonObjectRequest(Request.Method.GET,
            ...
            public void onResponse(JSONObject response) {
                ...
                DocumentTypesContainer documentTypesContainer;

                Type listType = new TypeToken<DocumentTypesContainer>() {
                }.getType();
                documentTypesContainer = gson.fromJson(response.toString(), listType);

                ArrayList<DocumentTypes> itemsDocumentTypes = documentTypesContainer.getItems();

                callback.onfinishedLoadDocumentTypes(itemsDocumentTypes);
            }
        }, new Response.ErrorListener() {

            @Override
            public void onErrorResponse(VolleyError error) {
                callback.onErrorLoadDocumentTypes();
