package com.itosoftware.inderandroid.Api.Eps;
...
public class ApiEps {
    ...
    EpsListener callback;
    Context context;

    public ApiEps(Fragment fragment){
        try {
            callback = (EpsListener) fragment;
        } catch (ClassCastException e) {
            throw new ClassCastException("Calling fragment must implement EpsListener interface");
        }
        this.context = fragment.getActivity().getApplicationContext();
    }

    public void getEps (HashMap params){
        ...
        String zones_list = context.getString(R.string.eps_list);
        ...
        JsonObjectRequest jsonObjReq = new JsonObjectRequest(Request.Method.GET,
            ...
            public void onResponse(JSONObject response) {
                ...
                Gson gson = new Gson();
                EpsContainer zonesContainer;

                Type listType = new TypeToken<EpsContainer>() {
                }.getType();
                zonesContainer = gson.fromJson(response.toString(), listType);

                ArrayList<Eps> itemsEps = zonesContainer.getItems();

                callback.onfinishedLoadEps(itemsEps);
            }
        }, new Response.ErrorListener() {

            @Override
            public void onErrorResponse(VolleyError error) {
                callback.onErrorLoadEps();
