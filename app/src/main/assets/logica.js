...
mostrarMarcadores = function(escenarios){
		
	if(parseInt(escenarios.totalItems) != 0){
		// Borrar Marcadores
		limpiar_marcadores(markers);
		limpiar_infowindows(infowindows);
		
		for(var i=0;i<escenarios.totalItems;i++){
			var escenario = escenarios.items[i];

			var id = escenario.id;
			var cx_latitude = escenario.latitude;
			var cy_longitude = escenario.longitude;
			var descripcion = escenario.descripcion;
			var info = escenario.info;
			var is_active = escenario.is_active;
			var imagen_url = escenario.imagen_url;
			...
			if(markers[j] == this){
			   ...
			   google.maps.event.addListener(infowindows[j], 'domready', function() {
			   	   $("#"+ infoId).click(function() {
						...
						Android.onSportsItemInfoWindowClick(parseInt(position[1]));
				   });
